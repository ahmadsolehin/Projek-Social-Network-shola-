...
//}
  ...
  //$u_check = mysqli_num_rows($query);
   ...
   // }
    ...
    //create a table with just id and username
    
    $sqlcv = "INSERT INTO cv (cv_id, username) VALUES ('$uid','$username')";
    $query = mysqli_query($link, $sqlcv);


    //     $sqlcertification = "INSERT INTO certification (id, username) VALUES ('$uid','$username')";
    // $query2 = mysqli_query($link, $sqlcertification);

    // $sqlemployment = "INSERT INTO employment (id, username) VALUES ('$uid','$username')";
    // $query3 = mysqli_query($link, $sqlemployment);
    

    // // $sqleducation = "INSERT INTO education (id, username) VALUES ('$uid','$username')";
    // // $query4 = mysqli_query($link, $sqleducation);


    //     $sqlprojects = "INSERT INTO projects (id, username) VALUES ('$uid','$username')";
    // $query5 = mysqli_query($link, $sqlprojects);



    
    // Create directory(folder) to hold each user's files(pics, MP3s, etc.)
  if (!file_exists('/home/grezzejn/dev/public_html/user_provider/'. $folder_name)) {
    mkdir('/home/grezzejn/public_html/dev/user_provider/'. $folder_name, 0755);
  ...
  $to = "$email";              
    ...
    $message = '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Grezzli Message</title></head><body style="margin:0px; font-family:Tahoma, Geneva, sans-serif;"><div style="padding:10px; background:#333; font-size:24px; color:#CCC;"><a href="http://dev.grezzli.com"><img src="http://dev.grezzli.com/img/logo.png" width="36" height="30" alt="Grezzli" style="border:none; float:left;"></a>Grezzli Account Activation</div><div style="padding:24px; font-size:17px;">Hello '.$first_name.',<br /><br />This is your username:  '.$username.',<br /><br /> Click the link below to activate your account when ready:<br /><br /><a href="http://dev.grezzli.com/activation.php?id='.$uid.'&u='.$username.'&p='.$pass_hash.'">Click here to activate your account now</a><br /><br />Login after successful activation using your:<br />* Username: <b>'.$username.'</b></div></body></html>';
    $headers = "From: $from\n";
