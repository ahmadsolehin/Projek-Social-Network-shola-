...
$config['callback_url']         =   'https://www.grezzli.com/service_seeker_reg.php'; //Your callback URL
...
if($return['error']) // if invalid output error
{
 $content = 'Some error occured<br><br>'.$return['error_description'].'<br><br>Please Try again.';
    ...
    else // token received successfully
    {
     ...
     //  print_r($User);
       ...
       if($count_emel!=0)
       {
        ...
        $sql = "INSERT INTO users (user_type, first_name, last_name, username, email, password, gender, country, city, avatar ,  signup, lastlogin, notescheck,   activated , security_question, security_answer)   VALUES('Service Seeker','$nama_pertama','$nama_akhir','$folder_nama','$emel','','','','', '$pictureUrls' ,now(),now(),now(),'1','' , '')";
    ...
    // Create directory(folder) to hold each user's files(pics, MP3s, etc.)
        if (!file_exists('/home/grezzejn/public_html/user_seeker/'. $folder_nama)) {
          mkdir('/home/grezzejn/public_html/user_seeker/'. $folder_nama, 0755);
  ...
  $redirect ='https://www.grezzli.com/service_seeker_reg.php';
  ...
  if (isset($accessToken)) {
    ...
    if($count!=0)
    {
      ...
      $sql = "INSERT INTO users (user_type, first_name, last_name, username, email, password, gender, country, city, avatar ,  signup, lastlogin, notescheck,   activated , security_question, security_answer)   VALUES('Service Seeker','$firstName','$lastName','$name','$email','','','','', '$image' ,now(),now(),now(),'1','' , '')";
    ...
    // Create directory(folder) to hold each user's files(pics, MP3s, etc.)
      if (!file_exists('/home/grezzejn/public_html/user_seeker/'. $folder_name)) {
        mkdir('/home/grezzejn/public_html/user_seeker/'. $folder_name, 0755);
  ...
  <div class="container">
    ...
    <div class="row">
      ...
      <div class="col-md-4 col-sm-4 col-xs-4">
        ...
        <form name="signup_provider_reg" action="php_includes/signup_service_seeker.php" role="form" method="post" autocomplete="off">
        ...
        <div class="form-group">
          ...
          <div class="input-group">
            ...
            <input id="emailchecker" name="InputEmail" placeholder="Enter email" class="form-control classname" autocomplete="off" readonly type="email" onfocus="if (this.hasAttribute('readonly')) { this.removeAttribute('readonly');
